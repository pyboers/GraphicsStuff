...
Shader * Shader::load(char * vsname, char * fsname) {
	...
	//glUseProgram(sd->programId);

	GLint length;
	// compile vertex shader
	length = (GLint)strlen(vs);
	glShaderSource(sd->vsId, 1, &vs, &length);
	glCompileShader(sd->vsId);

	// compile fragment shader
	length = (GLint)strlen(fs);
	glShaderSource(sd->fsId, 1, &fs, &length);
...
void Shader::uniformMat4(char * name, Mat4 * m) {
	glUniformMatrix4fv(getUniformLocation(name), 1, 0, m->mat);
}

void Shader::uniformVec3(char * name, Vector3f * v) {
